...
public @interface BackupSwagger {
    ...
    @Target({ElementType.METHOD})
    @Retention(RetentionPolicy.RUNTIME)
    @Operation(summary = "Criar backup do usuário", description = "Cria um backup dos dados do usuário")
    @ApiResponses(value = {
    ...
    @Target({ElementType.METHOD})
    @Retention(RetentionPolicy.RUNTIME)
    @Operation(summary = "Listar backups", description = "Lista todos os backups")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Backups listados com sucesso", content = { @Content(mediaType = "application/json", 
        schema = @Schema(implementation = BackupResponseDTO.class)) }),
        @ApiResponse(responseCode = "400", description = "Dados de backup inválidos",
                content = {@Content(mediaType = "application/json", schema = @Schema(implementation = StandardError.class))}),
        @ApiResponse(responseCode = "403", description = "Acesso negado", 
    ...
    @SecurityRequirement(name = "bearerAuth")
    @interface FindPageBackupSwagger {
    }
